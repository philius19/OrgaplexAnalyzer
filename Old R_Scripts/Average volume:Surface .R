...
csv_files <-list.files(pattern = "_Ly_Volume.csv", full.names = TRUE, recursive = TRUE)
...
for (file in csv_files) {
  ...
  volume_sum <- mean(csv_data[,1], na.rm = TRUE)
...
write.xlsx(total_volume_df, "Ly average_Volume.xlsx", row.names = FALSE)
